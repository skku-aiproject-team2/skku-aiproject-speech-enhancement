...
def evaluate_valen(testset_path, enhanced_path, target):
    reverb = 'no'
    result = defaultdict(int)
    wav_names = os.listdir(testset_path+"/clean")
    for i in tqdm(range(300)):
        try:
            rate, clean = wavfile.read(os.path.join(testset_path, "clean", wav_names[i]))
            if target == 'noisy':
                rate, target_wav = wavfile.read(os.path.join(testset_path, "noisy", wav_names[i]))
            else:
                rate, target_wav = wavfile.read(os.path.join(enhanced_path, wav_names[i]))
        except:
            continue
...
if __name__ == '__main__':
    ...
    elif args.dataset == 'valentini':
            result = evaluate_valen(testset_path, enhanced_path, target)    
    # logging
    for key in result:
        if key != 'count':
