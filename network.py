...
class CleanUNet_bilinear(nn.Module):
    ...
    def forward(self, noisy_audio):
        ...
        x = x[:, :, :L] * std
        return x
class CleanUNet_bilinear_v2(nn.Module):
    """ CleanUNet architecture. """
    ...
    def __init__(self, channels_input=1, channels_output=1,
        ...
        super(CleanUNet_bilinear_v2, self).__init__()
        ...
        print("The model is bilinear_v2")
        for i in range(encoder_n_layers):
            self.encoder.append(nn.Sequential(
                nn.Conv1d(channels_input, channels_H, kernel_size, stride, padding=1),
                nn.ReLU(),
            ...
            if i == 0:
                ...
                self.decoder.append(nn.Sequential(
                    ...
                    nn.GLU(dim=1),
                    nn.Upsample(scale_factor=stride, mode='linear', align_corners=False),
                    AverageChannels(factor=2),
                    nn.Conv1d(channels_H//2, channels_output, kernel_size, padding='same')
                ))
            else:
                self.decoder.insert(0, nn.Sequential(
                    nn.Conv1d(channels_H, channels_H * 2, 1), 
...
class CleanUNet_bilinear_lightConv(nn.Module):
    """ CleanUNet architecture. """
    ...
    def __init__(self, channels_input=1, channels_output=1,
        ...
        super(CleanUNet_bilinear_lightConv, self).__init__()
        ...
        print("The model is lightweight convolution")
        for i in range(encoder_n_layers):
            self.encoder.append(nn.Sequential(
                DepthwiseSeparableConv1d(channels_input, channels_H, kernel_size, stride, padding=1),
            ...
            if i == 0:
                self.decoder.append(nn.Sequential(
                    DepthwiseSeparableConv1d(channels_H, channels_H // 4, kernel_size=1),
                    nn.BatchNorm1d(channels_H // 4),
                    nn.ReLU(inplace=True),
                    nn.Upsample(scale_factor=stride, mode='linear', align_corners=False),
                    nn.Conv1d(channels_H // 4, channels_output, kernel_size=kernel_size, padding='same')
                ))
            else:
                self.decoder.insert(0, nn.Sequential(
                    DepthwiseSeparableConv1d(channels_H, channels_H // 4, kernel_size=1),
            ...
            channels_output = channels_H
            channels_H = min(channels_H * 2, max_H)
...
def match_tensor_size(tensor, target_size):
    if tensor.size(2) < target_size:
        padding_size = target_size - tensor.size(2)
        tensor = F.pad(tensor, (0, padding_size))
    elif tensor.size(2) > target_size:
        tensor = tensor[:, :, :target_size]
    return tensor


class CleanUNet_lightConv(nn.Module):
    """ CleanUNet architecture. """
    ...
    def __init__(self, channels_input=1, channels_output=1,
        ...
        super(CleanUNet_lightConv, self).__init__()
        ...
        for i in range(encoder_n_layers):
            ...
            if i == 0:
                self.decoder.append(nn.Sequential(
                    ...
                    nn.BatchNorm1d(channels_H // 4),
                    nn.ReLU(inplace=True),
                    nn.ConvTranspose1d(channels_H // 4, channels_output, kernel_size, stride)
                ))
            else:
                self.decoder.insert(0, nn.Sequential(
                    DepthwiseSeparableConv1d(channels_H, channels_H // 4, kernel_size=1),
    ...
    def forward(self, noisy_audio):
        ...
        for i, upsampling_block in enumerate(self.decoder):
            skip_i = skip_connections[i]
            skip_i = match_tensor_size(skip_i, x.size(2))
            # x = x + skip_i[:, :, :x.shape[-1]]
            x = x + skip_i
            x = upsampling_block(x)
